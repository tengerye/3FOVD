...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--csv_path",
        required=False,
        default='/home/ubuntu/workspace/GroundingDINO/car_categories.csv',
    )
    parser.add_argument(
        "--output_file",
        required=False,
        default='predict_jsons/groundingdino_car_test_prediction.json',
    )
    ...
    inference_for_all_images_and_captions(
        ...
        text_threshold,
        'cuda',              
        '/workspace/datasets/cars/test/instances_car_test.json',           # annotation JSON (COCO-format)
        '/workspace/datasets/cars/test/images',         # path to folder containing images
        args.csv_path,            # path to car_categories.csv
        args.output_file,          # path to the JSON file to store final results
        # 'vis_test/',
        # limit=10,
