...
class GroundingDINO(nn.Module):
    ...
    def forward_with_embeddings(self, srcs, masks, poss, text_dict, unset_image_tensor=True):
        """Forward pass using pre-computed embeddings."""
        self.set_image_features(srcs, poss)
...
def precompute_caption_embeddings(model, captions, device):
    captions = [caption.lower().strip() + "." if not caption.endswith(".") else caption.lower().strip() for caption in captions]
    ...
    tokenized_for_encoder = {k: v.to(device) for k, v in tokenized.items() if k != "attention_mask"}
    ...
    text_token_mask = tokenized.attention_mask.bool().to(device)
...
def precompute_image_embeddings(model, samples):
    if isinstance(samples, (list, torch.Tensor)):
        samples = nested_tensor_from_tensor_list(samples)
