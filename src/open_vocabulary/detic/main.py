...
def get_parser():
    ...
    parser.add_argument("--image_base_dir", required=False, default='images')
    parser.add_argument("--caption_idx", required=True)
    parser.add_argument("--out_file", required=False, default="detic_product_test.json")
    parser.add_argument("--text_embed_file", required=True)
    parser.add_argument(
...
def run_inference_directory_with_preprocessing(
    ...
    with torch.no_grad():
        for batch_inputs in tqdm.tqdm(data_loader):
    ...
    if rank == 0:
        ...
        for img_path, bboxes, labels_idx, scores in merged_results:
            ...
            img_and_cap_to_instances[img_filename][caption_idx] = {
                "boxes": bboxes,
                "labels": labels,
                "scores": scores,
            }
...
def visualize_demo(
        text_embed_file: str = "car_text_embeddings.json", 
        caption_idx = 1,
        vis_dir: str = 'test_vis',
    ) -> None:
    ...
    image_paths = glob("/home/ubuntu/workspace/datasets/cars/test/images/*.jpg", recursive=True)[:10]
...
if __name__ == "__main__":
    # visualize_demo("detic_car_text_embeddings.json")
    batch_demo(
        "detic_rp_text_embeddings.json"
    )
